...
class VedaBaseLabelTest(unittest.TestCase):
    ...
    def test_classification(self):
        ...
        label = ClassificationHandler._payload_handler(
            classification_item, klasses=['house', 'car'])
        self.assertEqual(label, [1, 1])

    def test_segmentation(self):
        from sampledata import segmentation_item
        label = SegmentationHandler._payload_handler(
            segmentation_item, klasses=['building'], out_shape=[256, 256])
    ...
    def test_object_detection(self):
        ...
        label = ObjDetectionHandler._payload_handler(
            objd_item, klasses=['building', 'damaged building'], out_shape=[256, 256])
