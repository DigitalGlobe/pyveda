...
class VedaCollectionTest(unittest.TestCase):
    ...
    def test_vedacollection(self):
        ...
        self.assertEqual(
            vc.__geo_interface__, box(
                *vc.bounds).__geo_interface__)


class VCFetchTest(unittest.TestCase):

    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch.yaml', filter_headers=['authorization'])
    def setUp(self):
        self.vc = pv.from_id(VC_ID)

    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_fetch_index.yaml', filter_headers=['authorization'])
    def test_fetch_index(self):
        dp = self.vc.fetch_sample_from_index(0)
        self.assertTrue(isinstance(dp, DataSampleClient))

    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_fetch_slice.yaml', filter_headers=['authorization'])
    def test_fetch_points(self):
        dps = self.vc.fetch_samples_from_slice(5, num_points=5)
        self.assertEqual(len(dps), 5)

    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_fetch_sample_id.yaml', filter_headers=['authorization'])
    def test_fetch_ids(self):
        dp = self.vc.fetch_sample_from_id(DP_ID)
        self.assertTrue(isinstance(dp, DataSampleClient))

    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_ids.yaml', filter_headers=['authorization'])
    def test_ids(self):
    ...
    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_ids_urls.yaml', filter_headers=['authorization'])
    ...
    @my_vcr.use_cassette(
        'tests/unit/cassettes/test_vcfetch_getatr.yaml', filter_headers=['authorization'])
