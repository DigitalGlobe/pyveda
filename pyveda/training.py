...
class BaseSet(object):
    ...
    def _load(self, geojson, image, **kwargs):
        ...
        meta.update({
            ...
            "rda_templates": self.rda_templates,
            "dtype": self.dtype
        })
        options = {
...
class VedaCollection(BaseSet):
    ...
    def __init__(self, name, mlType="classification", tilesize=[256,256], partition=[100,0,0], **kwargs):
        ...
        self.rda_templates = kwargs.get('rda_templates', [])
    ...
    def load(self, geojson, image, match="INTERSECT", default_label=None, label_field = None):
        ...
        self._update_templates(image)
        self._load(geojson, image, match=match, default_label=default_label, label_field=label_field )
    ...
    def _update_templates(self, image):
        """ Appends the template id to the list of already cached templates """
        #TODO fix this, it's just faked our for now
        self.rda_templates.append(image.__class__.__name__)
        self.rda_templates = list(set(self.sensors))
