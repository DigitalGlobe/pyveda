...
class WrappedDataNode(object):
    ...
    @property
    def images(self):
        return self._vset._image_array_factory(
            self._node.images, self._vset, output_transform=self._vset._fw_loader)

    @property
    def labels(self):
        return self._vset._label_array_factory(
            self._node.hit_table, self._node.labels, self._vset)
...
class H5DataBase(BaseDataSet):
    ...
    def _configure_instance(self, *args, **kwargs):
        ...
        self._classifications = dict([(klass,
                                       tables.UInt8Col(pos=idx + 1)) for idx,
                                      klass in enumerate(self.classes)])
    ...
    def _groups(self):
        return {
            group._v_name: group for group in self._fileh.root._f_iter_nodes("Group")}
