...
def build_vedabase(
    ...
    abf = VedaBaseFetcher(
        ...
        lbl_payload_handler=partial(
            database._label_klass._payload_handler,
            klasses=database.classes,
            out_shape=database.image_shape),
        num_lbl_payload_threads=label_threads,
        num_img_payload_threads=image_threads)
